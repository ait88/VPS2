...
configure_cloudflare_only() {
    # Implements: feat(nginx): Restrict direct server access to Cloudflare IPs only (#8)
    # Creates a geo block that identifies Cloudflare IPs and blocks direct access
    # to prevent attackers from bypassing Cloudflare's WAF and DDoS protection.

    info "Configuring Cloudflare-only access restriction..."

    local cf_only_conf="/etc/nginx/conf.d/cloudflare-only.conf"

    # Create geo block configuration with dynamically fetched Cloudflare IPs
    {
        echo "# Cloudflare-Only Access Control"
        echo "# Returns 403 for non-Cloudflare IPs"
        echo "# Auto-generated by setup-wordpress.sh - Updated $(date)"
        echo "#"
        echo "# This geo block sets \$cloudflare_ip to 1 for Cloudflare IPs"
        echo "# and 0 for all other IPs. Use in server blocks:"
        echo "#   if (\$cloudflare_ip = 0) { return 403; }"
        echo
        echo "geo \$realip_remote_addr \$cloudflare_ip {"
        echo "    default 0;"
        echo

        # IPv4 ranges from Cloudflare
        echo "    # IPv4 ranges"
        curl -s https://www.cloudflare.com/ips-v4 | while read ip; do
            [ -n "$ip" ] && echo "    $ip 1;"
        done
        ...
        echo
        echo "    # IPv6 ranges"
        # IPv6 ranges from Cloudflare
        curl -s https://www.cloudflare.com/ips-v6 | while read ip; do
            [ -n "$ip" ] && echo "    $ip 1;"
        done

        echo
        echo "    # Localhost (for health checks, WP-CLI loopback, local testing)"
        echo "    127.0.0.1 1;"
        echo "    ::1 1;"
        echo "}"
    } | sudo tee "$cf_only_conf" >/dev/null

    # Update the weekly cron to also update this file
    sudo tee /etc/cron.weekly/update-cloudflare-only >/dev/null <<'EOF'
#!/bin/bash
# Update Cloudflare-only access control IPs weekly

CF_ONLY_CONF="/etc/nginx/conf.d/cloudflare-only.conf"
TEMP_FILE="/tmp/cloudflare-only.tmp"
...
if [ -s "$TEMP_FILE" ]; then
    mv "$TEMP_FILE" "$CF_ONLY_CONF"
    nginx -t && systemctl reload nginx
fi
EOF

    sudo chmod +x /etc/cron.weekly/update-cloudflare-only

    success "Cloudflare-only access restriction configured"
    info "Non-Cloudflare IPs will receive 403 Forbidden"
}
...
generate_waf_restrictions() {
    ...
    case "$waf_type" in
        "none")
            ...
            ;;
        "cloudflare"|"cloudflare_ent")
            echo "    # Cloudflare-only access restriction (#8)"
            echo "    # Block direct access - only allow requests through Cloudflare"
            echo "    if (\\\$cloudflare_ip = 0) {"
            echo "        return 403;"
            echo "    }"
            echo
