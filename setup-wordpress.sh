...
mode_import_site() {
    ...
    for module in utils.sh preflight.sh config.sh users.sh database.sh wordpress.sh; do
        load_module "$module"
    done
...
mode_restore_backup() {
    ...
    for module in utils.sh preflight.sh backup.sh users.sh database.sh; do
        load_module "$module"
    done
